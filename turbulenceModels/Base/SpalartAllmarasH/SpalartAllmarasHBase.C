...
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::chi() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::fv1
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::fv2
(
    const volScalarField& chi,
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::ft2
(
    const volScalarField& chi
) const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::Omega
(
...
{
	// normalized helicity with limit to ensure it doesn't fail if denominator is zero
    return (mag((U)&fvc::curl(U))/(max(mag(U)*mag(fvc::curl(U)),dimensionedScalar("small", dimensionSet(0, 1, -2, 0, 0,0),SMALL))));
}

template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::r
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> SpalartAllmarasHBase<BasicEddyViscosityModel>::fw
(
    const volScalarField& Stilda,
    const volScalarField& dTilda
) const
...
// This is the function modified for the SAH formulation
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::Stilda
(
    const volScalarField& chi,
    const volScalarField& fv1,
    const volTensorField& gradU,
...
) const
{
    ...
    const volScalarField h(this->h(this->U_)); // normalized helicity

    return
        max
        (
        	// modified SAH expression
            (1.0*0.71*pow(h,0.6))*Omega + fv2(chi, fv1)*nuTilda_/sqr(kappa_*dTilda),
            Cs_*Omega
            // original:
...
template<class BasicEddyViscosityModel>
void SpalartAllmarasHBase<BasicEddyViscosityModel>::correctNut
(
    const volScalarField& fv1
)
{
...
template<class BasicEddyViscosityModel>
SpalartAllmarasHBase<BasicEddyViscosityModel>::SpalartAllmarasHBase
(
    const word& type,
...
template<class BasicEddyViscosityModel>
bool SpalartAllmarasHBase<BasicEddyViscosityModel>::read()
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField>
SpalartAllmarasHBase<BasicEddyViscosityModel>::DnuTildaEff() const
{
    return tmp<volScalarField>::New
    (
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::k() const
{
    // (B:Eq. 4.50)
...
template<class BasicEddyViscosityModel>
tmp<volScalarField>
SpalartAllmarasHBase<BasicEddyViscosityModel>::epsilon() const
{
    // (B:Eq. 4.50)
    const scalar Cmu = 0.09;
    const auto fv1 = this->fv1(chi());
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> SpalartAllmarasHBase<BasicEddyViscosityModel>::omega() const
{
    // (P:p. 384)
...
template<class BasicEddyViscosityModel>
void SpalartAllmarasHBase<BasicEddyViscosityModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
